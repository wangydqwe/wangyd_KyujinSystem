...
//2022.08.12 add end
    ...
    public String addKakunin(@PathVariable("rtype") Integer Rid,
        ...
        User user = (User) session.getAttribute("user");
        //UserId
        kaisha.setUserId(user.getUserId());
        //作成者
        kaisha.setSakuSeiSha(user.getUName());
        kyujin.setSakuSeiSha(user.getUName());
        //作成時間
        kaisha.setSakuSeiHiDuke(newdate);
        kyujin.setSakuSeiHiDuke(newdate);
        //更新者
        kaisha.setKouShinSha(user.getUName());
        kyujin.setKouShinSha(user.getUName());
        //更新時間
        kaisha.setKouShinHiDuke(newdate);
        kyujin.setKouShinHiDuke(newdate);
        //保存员工信息
    ...
    public String updateKakunin(@PathVariable("rtype") Integer Rid,
        ...
        try {
            ...
            model.addAttribute("kyujin",kyujin);
            model.addAttribute("oldkyujin",oldkyujin);
            model.addAttribute("rtype",Rid);
    ...
    public String updateKyushoku(@PathVariable("rtype") Integer Rid,
                                 Model model,
                                 HttpSession session,
                                 Kyujin kyujin)throws ParseException {
        //system date
        SimpleDateFormat formatter = new SimpleDateFormat( "yyyy-MM-dd HH:mm:ss");
        String date = formatter.format(new Date());
        Date newdate = formatter.parse(date);
