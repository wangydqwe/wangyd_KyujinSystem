...
//        List<Kyushoku> KyushokuList = kyushokuListService.kyushokuList();
    ...
    public String addKakunin(@PathVariable("rtype") Integer Rid,
        ...
        User user = (User) session.getAttribute("user");
        //UserId
        kojin.setUserId(user.getUserId());
        //作成者
        kojin.setSakuSeiSha(user.getUName());
        kyushoku.setSakuSeiSha(user.getUName());
        //作成時間
        kojin.setSakuSeiHiDuke(newdate);
        kyushoku.setSakuSeiHiDuke(newdate);
        //更新者
        kojin.setKouShinSha(user.getUName());
        kyushoku.setKouShinSha(user.getUName());
        //更新時間
        kojin.setKouShinHiDuke(newdate);
        kyushoku.setKouShinHiDuke(newdate);
        //保存员工信息
    ...
    public String updateKakunin(@PathVariable("rtype") Integer Rid,
        ...
        try {
            ...
            model.addAttribute("kyushoku",kyushoku);
            model.addAttribute("oldKyushoku",oldKyushoku);
            model.addAttribute("rtype",Rid);
    ...
    public String updateKyushoku(@PathVariable("rtype") Integer Rid,
                                 Model model,
                                 HttpSession session,
                                 Kyushoku kyushoku)throws ParseException {
        //system date
        SimpleDateFormat formatter = new SimpleDateFormat( "yyyy-MM-dd HH:mm:ss");
        String date = formatter.format(new Date());
        Date newdate = formatter.parse(date);
    ...
    public String deleteKyuShoku(@PathVariable("rtype") Integer Rid,
                                 Model model,
                                 HttpSession session,
                                 Kyushoku kyushoku)throws ParseException {
        //system date
