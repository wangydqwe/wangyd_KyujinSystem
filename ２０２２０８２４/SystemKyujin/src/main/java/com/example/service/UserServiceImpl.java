...
public class UserServiceImpl implements UserService{
    ...
    public User login(String username,String password,String mail){
        ...
        //2.判断mail
        if (!user.getMail().equals(mail)) throw new RuntimeException("メールまたはパスワードの入力エラー。もう一度入力してください。");
        //3.判断密码
        if (!user.getUPassword().equals(password)) throw new RuntimeException("メールまたはパスワードの入力エラー。もう一度入力してください。");
        return user;
    }
